...
export function HeroPage() {
  ...
  const bannerUrl = s?.hero?.banner_url || "";
  ...
  return (
    <div>
      ...
      <Card className="space-y-4">
        <Toggle label="Show Hero Section" checked={s?.hero?.enabled || false} onChange={(v) => set("hero.enabled", v)} testId="toggle-hero" />
        <Field label="Banner Image URL" testId="hero-banner" value={bannerUrl} onChange={(v) => set("hero.banner_url", v)} placeholder="/hero-banner.png or https://" />
        {bannerUrl && typeof bannerUrl === 'string' && (
          <img src={bannerUrl.startsWith("http") ? bannerUrl : bannerUrl} alt="preview" className="w-full rounded-[14px] border border-[#E5E7EB]" />
        )}
    ...
    <div>
      ...
      <div className="space-y-4">
        <Card>
          ...
          <div className="space-y-2">
            {(s?.sections || []).map((sec, i) => (
              <div key={sec?.id || i} data-testid={`section-row-${sec?.id}`} className="flex items-center gap-2 rounded-xl bg-[#F8F9FA] px-3 py-2">
                <span className="flex-1 text-sm font-medium text-[#111111]">{sec?.label || "Unknown Section"}</span>
                <label className="flex items-center gap-2 text-xs text-[#777777]">
                  <input type="checkbox" data-testid={`section-toggle-${sec?.id}`} checked={sec?.enabled || false} onChange={(e) => set(`sections.${i}.enabled`, e.target.checked)} className="h-4 w-4 accent-[#FFC107]" /> On
                </label>
                ...
                <button onClick={() => move(i, 1)} disabled={i === (s?.sections?.length || 0) - 1} className="flex h-7 w-7 items-center justify-center rounded-full bg-white text-[#555555] disabled:opacity-30"><ArrowDown className="h-3.5 w-3.5" /></button>
              </div>
            ))}
          </div>
        </Card>
        <Card className="space-y-3">
          ...
          <Toggle label="Show Stats" checked={s?.stats?.enabled || false} onChange={(v) => set("stats.enabled", v)} testId="toggle-stats" />
        ...
        <Card className="space-y-3">
          ...
          <Toggle label="Show Telegram CTA" checked={s?.telegram?.enabled || false} onChange={(v) => set("telegram.enabled", v)} testId="toggle-telegram" />
        ...
        <Card className="space-y-3">
          ...
          <Toggle label="Enable Ticker" checked={s?.winners_config?.enabled || false} onChange={(v) => set("winners_config.enabled", v)} testId="toggle-winners" />
          <Field label="Scroll Speed (sec)" type="number" value={s?.winners_config?.scroll_speed || 40} onChange={(v) => set("winners_config.scroll_speed", parseInt(v) || 40)} />
        </Card>
      </div>
    </div>
  );
...
export function CategoriesPage() {
  ...
  const add = () => { if (newCat.trim()) { setS((p) => ({ ...p, categories: [...(Array.isArray(p?.categories) ? p.categories : []), newCat.trim()] })); setNewCat(""); } };
...
export function AdsPage() {
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <Card className="space-y-4">
          <Toggle label="Enable Ads" checked={ads?.enabled || false} onChange={(v) => set("ads.enabled", v)} testId="toggle-ads" />
...
export function NotificationsPage() {
  ...
  return (
    <div>
      ...
      <Card className="space-y-4">
        <Toggle label="Show Announcement Bar" checked={s?.announcement?.enabled || false} onChange={(v) => set("announcement.enabled", v)} testId="toggle-announcement" />
