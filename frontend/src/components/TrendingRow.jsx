      ...
      <div className="no-scrollbar -mx-4 flex gap-3 overflow-x-auto px-4 pb-1">
        {safeApps.map((app, i) => (
          <motion.div
            ...
            className="relative w-[150px] shrink-0 cursor-pointer overflow-hidden rounded-[20px] border border-[#E5E7EB] bg-white p-3 shadow-[0_6px_24px_rgba(0,0,0,0.05)] transition-transform duration-200 active:scale-[0.97]"
          >
            {getBadge(app) && (
              <span data-testid={`app-badge-${app.id}`} className="absolute right-3 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-[#FFF3E0] font-display text-[11px] font-bold text-[#FF6F00]">
                {getBadge(app).label}
              </span>
            )}
            <AppIcon
            ...
            <h3 className="mt-2.5 line-clamp-1 font-display text-sm font-bold text-[#111111]">{app.name}</h3>
            <div className="mt-1 flex items-center gap-1 text-[11px] text-[#777777]">
            ...
            {app.signup_bonus && (
              <div data-testid={`trending-bonus-${app.id}`} className="mt-1.5 inline-flex items-center gap-1 rounded-[14px] border border-[#FFC107]/30 bg-gradient-to-r from-[#FFC107]/10 to-[#FF8300]/5 px-2 py-0.5 text-[9px] font-extrabold text-[#FF6F00]">
            ...
            <RippleButton
              ...
              className="mt-2.5 flex w-full items-center justify-center gap-1.5 rounded-full bg-[#FFC107] py-1.5 text-xs font-bold text-[#111111] shadow-[0_4px_12px_rgba(255,193,7,0.3)] hover:bg-[#FFB300]"
            >
